...
describe("Faucet ", function () {
  async function deployTokenFixture() {
    ...
    const faucet = await faucetContract.deploy(owner);
  ...
  it("native token faucet", async function () {
    const { owner, faucet, addr1 } = await loadFixture(deployTokenFixture);

    await expect(
      faucet
        .connect(owner)
        .withdrawTokens(addr1.address, [ethers.ZeroAddress], [100]),
    ).to.changeEtherBalances([faucet, addr1], [-100, 100]);
  });

  it("single token faucet", async function () {
    const { owner, faucet, hbtc, addr1 } =
      await loadFixture(deployTokenFixture);

    await expect(
      faucet
        .connect(owner)
        .withdrawTokens(addr1.address, [await hbtc.getAddress()], [100]),
    ).to.changeTokenBalance(hbtc, addr1, 100);
  });

  it("native and multiple token faucet", async function () {
    const { owner, faucet, hbtc, heth, addr1 } =
      await loadFixture(deployTokenFixture);
